...
Task #1 Add the title with an h2 tag above the table and change the content in the title tag of the index.html to the title indicated.

Task #2 The folder models is created to indicate the types of data desired for the delivered data. The helpers folder is created, in which there is a function to perform the parsing to the required data types, also the missing numeric data is assumed as 0 to be able to sort them better. Now with this we add the props of sorting and filters to the columns, we also allow the multiple sorting with the ctrl key, this, adding the prop multiSortKey and assigning the ctrl key to it.

Task #3 Using the prop valueFormatter we format the date day/month/year, which were considered more priority than the day of the week and the time.

Task #4 In this case we used the parse function performed in task #2 so that by means of a ternary operator we can choose the word to replace.

Task #5 According to the documentation this task would be possible by using the enableRangeSelection prop with status set to true, but this prop is only enabled with 'ag-grid-enterprise', which is not included in the libraries previously installed in the project.

Task #6 A reference to the table is used to be able to access it from the created button and use the functions that allow to set the values of the filters and sorting to their initial state.

Task #7 To maintain an order we have the components folder, where the components to use and their css will go, the data folder, where is the data used to fill the table, the helpers folder, for the reusable functions for the project and the models folder, where is the model of the data types that are in the table data.